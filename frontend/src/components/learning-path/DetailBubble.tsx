  ...
  >
    <div className="bg-[#fbb124] text-white rounded-xl px-6 py-3 shadow-lg relative w-60" style={blockedStyle}>
      <div className="absolute left-1/2 -top-2 -translate-x-1/2">
        <div className="w-6 h-6 bg-[#fbb124] rotate-45" style={blockedStyle}/>
      </div>
      <div className="mb-2"><span className="font-bold text-lg">{title}</span></div>
      <div className="mb-3 text-base">{subtitle}</div>
      <button
        className="flex rounded-full px-14 py-2 gap-1.5 text-lg w-full text-center justify-center transition hover:bg-[#fff3d1] active:scale-95"
        style={style}
        onClick={onClick}
      >
