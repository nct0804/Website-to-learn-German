...
export default function Pronunciation() {
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      try {
        setLoading(true);
        const res = await fetch('http://localhost:3000/api/vocabulary/groups', {
          headers: { Cookie: 'refreshToken=4c693e4b-4c1a-495a-a4b2-55fcd1d563d1' },
        });
              ...
              {group.sounds.map(({ sound }) => {
                ...
                return (
                  <button
                    ...
                    disabled={isPlaying}
                    className={
                      `relative overflow-hidden bg-white rounded-lg shadow-sm transform transition-transform ease-in-out duration-200 active:scale-95 hover:scale-105 cursor-pointer border border-gray-100 hover:border-gray-200 p-3 text-center will-change-[transform,opacity]`
                    }
                    style={{ minHeight: '120px' }}
                  >
                    <div className={`absolute inset-0 bg-gradient-to-br ${gradient} transition-opacity duration-300 ease-in-out ${isActive ? 'opacity-10' : 'opacity-0'}`} />
                    <div className="relative z-10 h-full flex flex-col justify-between">
                      <div className={`text-2xl font-bold mb-1 bg-clip-text text-transparent bg-gradient-to-r ${gradient} transition-transform duration-200 ease-in-out ${isActive ? 'scale-105' : ''}`}> 
                        {sound.symbol}
                      </div>
                      <div className="mb-2 flex-1 flex flex-col justify-center">
                        ...
                        <div className="text-xs font-semibold text-gray-700">{sound.exampleWord}</div>
                      </div>
                      <div className={`inline-flex items-center justify-center w-6 h-6 mx-auto rounded-full transition-transform duration-200 ease-in-out ${isActive ? `bg-gradient-to-r ${gradient} text-white shadow-md scale-105` : 'bg-gray-100 text-gray-400'}`}>
                        <Play className={`w-2.5 h-2.5 transition-transform duration-200 ease-in-out ${isActive ? 'animate-pulse scale-105' : ''}`} />
                      </div>
                    </div>
                    {isActive && (
                      <div className="absolute inset-0 rounded-lg pointer-events-none">
                        <div className={`absolute inset-0 rounded-lg bg-gradient-to-r ${gradient} opacity-20 animate-ping`} 
                          style={{ animationDuration: '1000ms', animationTimingFunction: 'cubic-bezier(0.4,0,0.6,1)' }}
                        />
                      </div>
                    )}
                  </button>
